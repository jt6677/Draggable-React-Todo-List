...
export const useArrowKeyFocus = (todos: TodoType[]) => {
  const { focusedElement, setFocusedElement } = useTodoListStore((state) => ({
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      if (event.key === 'ArrowUp' || event.key === 'ArrowDown') {
        ...
        const currentIndex = allItems.findIndex(
          (item) =>
            `${item.parentId !== undefined ? item.parentId : ''}${item.id}textfield` ===
            focusedElement
        )
        ...
        if (targetIndex !== -1) {
          ...
          const targetElement = document.getElementById(
            `${targetParentId !== undefined ? targetParentId : ''}${targetId}textfield`
